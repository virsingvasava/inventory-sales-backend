   ...
   .from-group .cityd.droup-select .nice-select.open .list {
    ...
    <div class="main-content-part">
        <div class="main-content-padd">
            <form action="{{ route('airport_manager.profile.profile_update') }}" enctype="multipart/form-data" method="POST"
                id="profile_update">
                @csrf
                        ...
                        <div class="col-md-2">
                            <div class="photo-upload-sec">
                                @if ($user_detail->profile_img != '' &&
                                    file_exists(public_path('assets/profile_picture/' . $user_detail->profile_img)))
                                    <figure><img id="current_login_user_picture"
                                            src="{{ asset('assets/profile_picture/' . $user_detail->profile_img) }}" />
                                    </figure>
                                @else
                                    <figure><img id="current_login_user_picture"
                                            src="{{ asset('theme/images/profile-pic.png') }}" /></figure>
                                @endif
                                ...
                                <div class="col-md-6">
                                    <div class="from-group">
                                        ...
                                        <div class="droup-select cityd">
                                            <input type="hidden" name="city_id" value="{{ $user_detail->city_id }}" />
                                            <select class="form-control custom-select" id="citySelect" name="city_id" disabled>
                                                <option value="">All Cities</option>
                                                @if (!empty($cityArray) && count($cityArray) > 0)
                                                    @foreach ($cityArray as $key => $city)
                                                        <option value="{{ $city->id }}"
                                                            {{ $city->id == $user_detail->city_id ? 'selected' : '' }}>
                                                            {{ $city->name }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                    ...
                                    <div class="from-group">
                                        ...
                                        <div class="droup-select">
                                            ...
                                            <select class="form-control nice-select" name="role_id" disabled>
                                                ...
                                                <option @if ($role == 2) selected="selected" @endif
                                                    value="{{ AIRPORT_MANAGER }}">Airport Manager</option>
                                                <option @if ($role == 3) selected="selected" @endif
                                                    value="{{ BRANCH_MANAGER }}">Branch Manager</option>
                                                <option @if ($role == 4) selected="selected" @endif
                                                    value="{{ HO }}">HO</option>
                                                <option @if ($role == 5) selected="selected" @endif
                                ...
                                <div class="col-md-4">
                                    <div class="from-group">
                                        ...
                                        <input type="date" name="date_of_joining"
                                            value="{{ $user_detail->date_of_joining }}" readonly>
                                    ...
                                    <div class="from-group">
                                        ...
                                        <div class="droup-select">
                                            <select class="form-control custom-select nice-select" name="status" disabled>
                                                ...
                                                <option @if ($user_detail->status == 1) selected="selected" @endif
                                                    value="1">Active</option>
                                                <option @if ($user_detail->status == 0) selected="selected" @endif
